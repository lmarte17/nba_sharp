...
try:
    ...
    from stats_retrieval.fetch_and_merge_player_stats import (
    ...
    from stats_retrieval.fetch_and_merge_player_stats import (  # type: ignore
        fetch_and_merge_player_stats,
    )
...
def run(season: str, season_type: str, per_mode: str, database_url: str) -> None:
    for timeframe, last_n in TIMEFRAME_TO_LAST_N.items():
        print(f"Fetching player stats for timeframe '{timeframe}' (last_n_games={last_n})...")
        ...
        table_name = f"player_stats_{timeframe}"
        print(f"Writing {len(df)} rows to player_data.{table_name}...")
        upsert_dataframe(df, table_name, database_url)
        print(f"Wrote player_data.{table_name}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest NBA player stats into Postgres tables for multiple timeframes."
    )
